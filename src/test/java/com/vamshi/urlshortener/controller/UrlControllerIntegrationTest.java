...
class UrlControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    void shortenUrl_customAlias_roundTrip() throws Exception {
        String body = objectMapper.writeValueAsString(
                new CreateUrlRequest("https://example.com", "mytestctrl", null));

        mockMvc.perform(post("/api/v1/urls")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.shortCode").value("mytestctrl"));

        mockMvc.perform(get("/mytestctrl"))
                .andExpect(status().isFound())
    ...
    void shortenUrl_duplicateAlias_returns409() throws Exception {
        String body = objectMapper.writeValueAsString(
                new CreateUrlRequest("https://example.com", "taken1ctrl", null));
